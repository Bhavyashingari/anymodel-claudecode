...
const PROVIDERS = ['openrouter', 'ollama', 'openai', 'azure', 'vertex'];
...
export async function detectProvider(model) {
  ...
  if (process.env.VERTEX_PROJECT_ID || process.env.VERTEX_ACCESS_TOKEN) return 'vertex';
...
${C.magenta('  anymodel')} — universal AI coding tool
  ...
  ${C.bold('Environment:')}
    ...
    AZURE_OPENAI_API_KEY    API key for Azure OpenAI endpoints
    AZURE_OPENAI_ENDPOINT   Base endpoint for Azure OpenAI
    VERTEX_PROJECT_ID       Google Cloud Project ID
    VERTEX_LOCATION        Google Cloud Location (default: us-central1)
    VERTEX_ACCESS_TOKEN    Google Cloud Access Token (from gcloud auth)
